...
describe('renderEventText', () => {
  ...
  it('renders member events without a channel', () => {
    expect(renderEventText('member.joined', { user: { username: 'bob' } }))
      .toBe('👋 @bob joined the server');
    expect(renderEventText('member.removed', { user: { username: 'bob' }, reason: 'spam' }))
      .toBe('🚫 @bob was removed from the server — spam');
  });
