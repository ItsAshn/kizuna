...
function getMessageInfo(
  ...
  channel_id: string
  isDM: boolean
  participants?: { user1_id: string; user2_id: string }
} | null {
  const msg = db.prepare('SELECT channel_id FROM messages WHERE id = ?').get(messageId) as
    | { channel_id: string }
    | undefined
...
function getReactionsForMessage(
  ...
  for (const r of rows) {
    const existing = reactions.find(
      (e) => e.reaction_key === r.reaction_key && e.reaction_type === r.reaction_type,
    )
    if (existing) {
      existing.count++
...
reactionRoutes.delete('/:messageId/:reactionKey', authMiddleware, (c) => {
  ...
  db.prepare(
    'DELETE FROM message_reactions WHERE message_id = ? AND user_id = ? AND reaction_key = ?',
