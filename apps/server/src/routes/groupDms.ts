...
groupDmRoutes.post('/', authMiddleware, async (c) => {
  ...
  const channel = db
    .prepare('SELECT * FROM group_dm_channels WHERE id = ?')
    .get(id) as GroupDMChannelRow
...
groupDmRoutes.get('/:channelId', authMiddleware, (c) => {
  ...
  const channel = db.prepare('SELECT * FROM group_dm_channels WHERE id = ?').get(channelId) as
    | GroupDMChannelRow
    | undefined
...
groupDmRoutes.patch('/:channelId', authMiddleware, async (c) => {
  ...
  if (body.name !== undefined) {
    ...
    db.prepare('UPDATE group_dm_channels SET name = ? WHERE id = ?').run(
      body.name.trim(),
      channelId,
    )
  }
  if (body.avatar !== undefined) {
    db.prepare('UPDATE group_dm_channels SET avatar = ? WHERE id = ?').run(
      body.avatar || null,
      channelId,
    )
  }
  ...
  const updated = db
    .prepare('SELECT * FROM group_dm_channels WHERE id = ?')
    .get(channelId) as GroupDMChannelRow
  ...
  try {
    ...
    if (io) {
      const members = db
        .prepare('SELECT user_id FROM group_dm_members WHERE channel_id = ?')
...
groupDmRoutes.get('/:channelId/messages', authMiddleware, (c) => {
  ...
  if (before) {
    const anchor = db
      .prepare('SELECT created_at FROM group_dm_messages WHERE id = ?')
    ...
    rows = anchor
      ? (db
        ...
        `,
          ...
          .all(channelId, anchor.created_at, limit) as {
          id: string
          channel_id: string
          from_id: string
          from_username: string
...
groupDmRoutes.post('/:channelId/messages', authMiddleware, async (c) => {
  ...
  const userRow = db
    ...
    .get(user.userId) as { display_name: string | null; avatar: string | null } | undefined
  const message = {
    id,
    channel_id: channelId,
    user_id: user.userId,
  ...
  try {
    ...
    if (io) {
      const members = db
        .prepare('SELECT user_id FROM group_dm_members WHERE channel_id = ? AND user_id != ?')
...
groupDmRoutes.delete('/messages/:messageId', authMiddleware, (c) => {
  ...
  try {
    ...
    if (io) {
      ...
      for (const m of members) {
        io.to(`group-dm:${m.user_id}`).emit('group-dm:delete', {
          id: messageId,
...
groupDmRoutes.patch('/messages/:messageId', authMiddleware, async (c) => {
  ...
  const now = Math.floor(Date.now() / 1000)
  db.prepare('UPDATE group_dm_messages SET content = ?, edited_at = ? WHERE id = ?').run(
    content.trim(),
    now,
    messageId,
  )
...
groupDmRoutes.post('/:channelId/members', authMiddleware, async (c) => {
  ...
  const currentCount = db
    .prepare('SELECT COUNT(*) as count FROM group_dm_members WHERE channel_id = ?')
  ...
  const now = Math.floor(Date.now() / 1000)
  db.prepare('INSERT INTO group_dm_members (channel_id, user_id, joined_at) VALUES (?, ?, ?)').run(
    channelId,
    userId,
    now,
  )
...
groupDmRoutes.delete('/:channelId/members/:userId', authMiddleware, (c) => {
  ...
  if (isOwner && isSelf) {
    ...
    db.prepare(
      'DELETE FROM message_reactions WHERE message_id IN (SELECT id FROM group_dm_messages WHERE channel_id = ?)',
    ).run(channelId)
  ...
  db.prepare('DELETE FROM group_dm_members WHERE channel_id = ? AND user_id = ?').run(
    channelId,
    targetUserId,
  )
  ...
  try {
    const io = getIo(c)
    if (io) {
      io.to(`group-dm:${targetUserId}`).emit('group-dm:member-removed', {
        channel_id: channelId,
      ...
      for (const m of remainingMembers) {
        io.to(`group-dm:${m.user_id}`).emit('group-dm:member-left', {
          channel_id: channelId,
          user_id: targetUserId,
        })
      }
...
adminGroupDmRoutes.get('/stats', authMiddleware, adminMiddleware, (c) => {
  ...
  const totalChannels = db.prepare('SELECT COUNT(*) as count FROM group_dm_channels').get() as {
    count: number
  }

  const totalMembers = db.prepare('SELECT COUNT(*) as count FROM group_dm_members').get() as {
    count: number
  }

  const activeSessions = db
    .prepare('SELECT COUNT(*) as count FROM group_dm_voice_participants WHERE left_at IS NULL')
    .get() as { count: number }
