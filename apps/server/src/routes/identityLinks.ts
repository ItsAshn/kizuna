...
identityLinkRoutes.post('/identity-link/confirm', authMiddleware, async (c) => {
  ...
  const { requestingServer, nonce } = (await c.req.json()) as {
  ...
  const user = db
    ...
    .get(auth.userId) as { username: string; display_name: string } | undefined
  if (!user) {
  ...
  db.prepare(
    ...
    user.username,
    user.display_name,
    normalizeUrl(requestingServer.trim()),
    now,
  )
...
identityLinkRoutes.post(
  '/identity-link/complete',
  sensitiveAuthLimiter,
  authMiddleware,
  async (c) => {
    const auth = getAuth(c)
    const { linkedServerUrl, verificationToken } = (await c.req.json()) as {
    ...
    try {
      const response = await fetch(
        `${serverUrl}/api/auth/identity-link/verify/${encodeURIComponent(verificationToken.trim())}`,
        {
      ...
      if (!response.ok) {
        ...
        return c.json(
          {
            error: (body as { error?: string }).error || 'Verification failed on the linked server',
          },
          400,
        )
      }
...
identityLinkRoutes.patch('/identity-links/:id/public', authMiddleware, async (c) => {
  ...
  const db = getDb()
  const link = db
    .prepare('SELECT id FROM identity_links WHERE id = ? AND user_id = ?')
    .get(linkId, auth.userId) as { id: string } | undefined
