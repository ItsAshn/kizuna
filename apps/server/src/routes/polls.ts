...
function parsePollBody(body: unknown): PollCreateInput | string {
  ...
  const options: string[] = b.options
    .map((o) => String(o).trim())
    .filter(Boolean)
    .slice(0, 10)
...
function insertPoll(
  ...
  const author = db.prepare('SELECT display_name, avatar FROM users WHERE id = ?').get(userId) as
  ...
  options.forEach((label, i) => {
    const optId = uuidv4()
    db.prepare(`INSERT INTO poll_options (id, poll_id, label, position) VALUES (?, ?, ?, ?)`).run(
      optId,
      pollId,
      label,
      i,
...
function listChannelPolls(
  ...
  return rows.map((poll) => {
    ...
    const author = db
      ...
      .get(poll.created_by) as { display_name: string; avatar: string | null } | undefined
    const userVotes = db
      .prepare('SELECT option_id FROM poll_votes WHERE poll_id = ? AND user_id = ?')
      .all(poll.id, userId) as { option_id: string }[]
...
pollsRouter.post('/channels/:channelId/polls', authMiddleware, async (c) => {
  ...
  return c.json({
    poll: { id: pollData.pollId, question: pollData.question, options: pollData.options },
  })
})
...
pollsRouter.post('/dms/channel/:channelId/polls', authMiddleware, async (c) => {
  ...
  const channel = db
    ...
    .get(channelId, userId, userId) as Record<string, unknown> | undefined
  if (!channel) return c.json({ error: 'Channel not found' }, 404)
  ...
  const toId =
    channel.user1_id === userId ? (channel.user2_id as string) : (channel.user1_id as string)
...
pollsRouter.get('/polls/:pollId', authMiddleware, async (c) => {
  ...
  const userVotes = db
    .prepare('SELECT option_id FROM poll_votes WHERE poll_id = ? AND user_id = ?')
    .all(pollId, userId) as { option_id: string }[]
...
pollsRouter.post('/polls/:pollId/vote', authMiddleware, async (c) => {
  ...
  try {
    db.prepare('INSERT INTO poll_votes (id, poll_id, option_id, user_id) VALUES (?, ?, ?, ?)').run(
      uuidv4(),
      pollId,
      body.optionId,
      userId,
    ...
    db.prepare('DELETE FROM poll_votes WHERE poll_id = ? AND option_id = ? AND user_id = ?').run(
      pollId,
      body.optionId,
      userId,
    )
  ...
  if (io) {
    if (poll.channel_type === 'dm') {
      const dmChannel = db
        .prepare('SELECT user1_id, user2_id FROM dm_channels WHERE id = ?')
        .get(poll.channel_id) as { user1_id: string; user2_id: string } | undefined
      if (dmChannel) {
      ...
      const members = db
        ...
        .all(poll.channel_id) as { user_id: string }[]
      for (const m of members) {
