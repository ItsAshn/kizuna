...
export function registerCallHandlers(io: Server, socket: Socket): void {
  ...
  socket.on('dm:call:start', ({ dmChannelId }: { dmChannelId: string }, callback?: Function) => {
    ...
    if (!userInfo || !hasPermission(userInfo, 'initiate_dm_calls')) {
      if (typeof callback === 'function')
        callback({ error: 'You do not have permission to start DM calls' })
      return
    }
    const db = getDb()
    const dmChannel = db
      .prepare('SELECT * FROM dm_channels WHERE id = ? AND (user1_id = ? OR user2_id = ?)')
      .get(dmChannelId, userId, userId) as
    ...
    const calleeRow = db.prepare('SELECT username FROM users WHERE id = ?').get(calleeId) as
      | { username: string }
      | undefined
  ...
  socket.on('dm:call:accept', ({ dmChannelId }: { dmChannelId: string }, callback?: Function) => {
    ...
    if (!userInfo || !hasPermission(userInfo, 'initiate_dm_calls')) {
      if (typeof callback === 'function')
        callback({ error: 'You do not have permission to accept DM calls' })
      return
    }
  ...
  socket.on('group-dm:call:start', ({ channelId }: { channelId: string }, callback?: Function) => {
    ...
    if (!userInfo || !hasPermission(userInfo, 'initiate_dm_calls')) {
      if (typeof callback === 'function')
        callback({ error: 'You do not have permission to start calls' })
      return
    }
    const db = getDb()
    const isMember = db
  ...
  socket.on('group-dm:call:accept', ({ channelId }: { channelId: string }, callback?: Function) => {
    ...
    if (!userInfo || !hasPermission(userInfo, 'initiate_dm_calls')) {
      if (typeof callback === 'function')
        callback({ error: 'You do not have permission to accept calls' })
      return
    }
