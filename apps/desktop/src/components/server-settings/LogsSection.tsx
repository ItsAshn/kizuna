...
import { fetchStorageStats, clearAuditLogs, cleanupOrphanFiles } from '@kizuna/shared'
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <p style={{ color: 'var(--text-secondary)', fontSize: '13px', margin: 0 }}>
        Manage server storage and audit logs. Auto-cleanup runs every 6 hours for orphan files and
        daily for old audit logs.
      </p>
      ...
      {stats && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
          ...
          >
            ...
            <div style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>
              {formatBytes(stats.gifs.totalSize)}
            </div>
          </div>
          <div
            style={{
          ...
          >
            ...
            <div style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>
              {formatBytes(stats.orphans.totalSize)}
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
        ...
        >
          <span style={{ flex: 1, fontSize: '13px' }}>
            Clear audit logs ({stats?.auditLogs.count ?? 0} entries)
          </span>
          {clearConfirm ? (
            <>
              <button
        ...
        >
          <span style={{ flex: 1, fontSize: '13px' }}>
            Clean up orphaned files ({stats?.orphans.count ?? 0} files,{' '}
            {stats ? formatBytes(stats.orphans.totalSize) : '0 B'})
          </span>
          {orphanConfirm ? (
            <>
              <button
              ...
              <button
                className="server-menu__btn"
                onClick={() => setOrphanConfirm(false)}
                style={{ padding: '4px 10px', fontSize: '12px' }}
              >
                cancel
              </button>
      ...
      {actionMsg && (
        <p
          style={{
            color: actionMsg.startsWith('error') ? 'var(--error)' : 'var(--success)',
            fontSize: '13px',
          }}
        >
          {actionMsg}
