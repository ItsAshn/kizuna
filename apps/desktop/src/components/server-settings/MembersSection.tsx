...
export function MembersSection({ serverUrl }: { serverUrl: string | undefined }) {
  ...
  useEffect(() => {
    ...
    fetchMembers(serverUrl)
      .then((d) => {
        if (mountedRef.current) setMembers(d)
      })
      .catch(console.error)
      .finally(() => {
  ...
  const handleBulkAddRole = async (roleId: string) => {
    ...
    const targets = members.filter(
      (m) => selectedMembers.has(m.id) && !(m.custom_roles || []).some((r) => r.id === roleId),
    )
    for (const m of targets) {
      try {
  ...
  return (
    <div className="server-menu__section">
        ...
        <div className="server-menu__bulk-bar">
          ...
          <div className="server-menu__bulk-bar-actions">
            ...
            <button
              ...
              className="server-menu__bulk-bar-btn server-menu__bulk-bar-btn--danger"
            >
              kick
            </button>
            {roles.length > 0 && (
              <select
                value=""
                onChange={(e) => {
                  if (e.target.value) handleBulkAddRole(e.target.value)
                  e.target.value = ''
                }}
                className="server-menu__bulk-bar-btn"
                style={
      ...
      {selfDemoteConfirm &&
        selectedMembers.size > 0 &&
        selectedMembers.has(session?.user?.id ?? '') && (
          <div className="server-menu__self-warn" style={{ marginBottom: '12px' }}>
            ...
            <div style={{ display: 'flex', gap: '6px', marginTop: '6px' }}>
              <button
                onClick={() => handleBulkToggleAdmin(false)}
                className="server-menu__confirm-btn"
              >
                confirm demotion
              </button>
              <button
                onClick={() => setSelfDemoteConfirm(false)}
                className="server-menu__confirm-cancel"
              >
                cancel
              </button>
      ...
      {membersLoading ? (
        ...
        filteredMembers.map((m) => {
          ...
          return (
            <div key={m.id}>
              <div
                className={`server-menu__member${isSelected ? ' server-menu__member--selected' : ''}`}
              >
                <label className="server-menu__member-checkbox">
                  <input
                    ...
                    checked={isSelected}
                    onChange={() => {
                      if (!m.is_host) toggleMemberSelect(m.id)
                    }}
                    disabled={m.is_host}
                  />
                ...
                <div className="server-menu__member-avatar-wrap">
                  <div className="server-menu__member-avatar">
                    ...
                    {m.avatar && (
                      <img
                        src={m.avatar}
                        alt=""
                        className="server-menu__member-avatar-img"
                        onError={(e) => {
                  ...
                  {status !== 'offline' && (
                    <span
                      className={`server-menu__status-dot server-menu__status-dot--${status}`}
                    />
                  )}
                </div>
                <div className="server-menu__member-info">
                  <p className="server-menu__member-name">
                    ...
                    {m.is_host && (
                      <span
                        className="server-menu__member-badge"
                        style={{
                          color: '#fab005',
                          borderColor: hexToRgba('#fab005', 0.4),
                          backgroundColor: hexToRgba('#fab005', 34 / 255),
                          marginLeft: '4px',
                        }}
                      >
                        host
                ...
                {memberMsg[m.id] && (
                  <span className="server-menu__member-msg">{memberMsg[m.id]}</span>
                )}
                {m.reset_requested_at && (
                  <span
                    ...
                    style={{
                      color: '#fbbf24',
                      borderColor: hexToRgba('#fbbf24', 0.4),
                      backgroundColor: hexToRgba('#fbbf24', 34 / 255),
                    }}
                  >
                    reset
                  </span>
                ...
                <div
                  className="server-menu__member-overflow-wrap"
                  ref={openOverflowMember === m.id ? overflowRef : undefined}
                >
                  <button
                    onClick={() => {
                        ...
                        <>
                          ...
                          {roles.map((r) => {
                            ...
                            return (
                              ...
                              >
                                <span
                                  className="server-menu__overflow-role-dot"
                                  style={{ backgroundColor: r.color }}
                                />
                                {r.name}
                                {hasRole && (
                                  <span className="server-menu__overflow-role-check">&#10003;</span>
                                )}
                              </button>
                            )
                          })}
              ...
              {kickConfirmMember === m.id && (
                <div className="server-menu__confirm-panel">
                  ...
                  <div className="server-menu__confirm-actions">
                    <button onClick={() => handleKick(m)} className="server-menu__confirm-btn">
                      kick
                    </button>
                    <button
              ...
              {resetTokenData?.userId === m.id && (
                ...
                >
                  <p style={{ color: 'var(--text-muted)', marginBottom: '4px' }}>
                    Reset token for {resetTokenData.username} (valid 24h) — share this with the
                    user:
                  </p>
