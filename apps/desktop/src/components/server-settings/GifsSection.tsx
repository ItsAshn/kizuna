...
export function GifsSection({ serverUrl }: { serverUrl: string | undefined }) {
  ...
  const [taggerStatus, setTaggerStatus] = useState<TaggerStatus>({
    loaded: false,
    loading: false,
    enabled: false,
  })
  ...
  const [packNameModal, setPackNameModal] = useState<{
    open: boolean
    file: File | null
    name: string
  ...
  const handleGifFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      await uploadGif(
        ...
        file,
        name,
        gifCategory.trim() || undefined,
        gifTags.trim() || undefined,
      )
      setGifMsg('uploaded')
      setGifName('')
      setGifCategory('')
    ...
    <>
      <div className="server-menu__section">
        ...
        {gifTab === 'gif' && (
          <>
            <p
              className="server-menu__section-title"
            ...
            <div className="server-menu__gif-upload">
              ...
              <button
                ...
                disabled={gifUploading || !gifName.trim()}
            ...
            <p
              className="server-menu__section-title"
              style={{ marginTop: '16px', marginBottom: '6px', fontSize: '11px' }}
            >
              upload gif pack (.zip)
            </p>
            <div className="server-menu__gif-upload" style={{ marginBottom: '16px' }}>
              ...
              <button
                onClick={() => gifPackFileRef.current?.click()}
              ...
              <input
                ...
                accept=".zip"
                style={{ display: 'none' }}
                onChange={handleGifPackFile}
              />
            </div>

            {gifMsg && (
              <span
                className={`server-menu__save-msg ${gifMsg === 'uploaded' || gifMsg.startsWith('imported') ? 'server-menu__save-msg--ok' : 'server-menu__save-msg--err'}`}
              ...
              >
                {showTaggerWarning ? (
                  <>
                    ...
                    <button
                      onClick={() => setShowTaggerWarning(false)}
                      className="server-menu__upload-btn"
                      style={{ fontSize: '11px', padding: '4px 8px' }}
                    >
                      cancel
            ...
            {taggerStatus.loading && (
              <p style={{ fontSize: '11px', color: '#888', margin: '0 0 8px 0' }}>
                loading tagging model... this may take a minute
              </p>
            )}
            {taggerStatus.loaded && (
              <div
            ...
            <p
              ...
              style={{ marginBottom: '6px', fontSize: '11px' }}
            >
              gif library ({gifsList.filter((g) => g.type === 'gif').length})
            </p>
            {taggerStatus.loaded && (
              <button
            ...
            {gifsLoading ? (
              ...
              <div className="server-menu__gif-grid">
                {gifsList
                  .filter((g) => g.type === 'gif')
                  .map((gif) => {
                    const resolvedUrl = gif.file_url.startsWith('/')
                      ? `${serverUrl}${gif.file_url}`
                      : gif.file_url
                    ...
                    return (
                      <div key={gif.id} className="server-menu__gif-item">
                        <img
                          src={resolvedUrl}
                          alt={gif.display_name}
                          className="server-menu__gif-thumb"
                        />
                        ...
                        {isEditing && (
                          <div className="server-menu__gif-popover">
                            <input
                              value={gifEditName}
                              onChange={(e) => setGifEditName(e.target.value)}
                              placeholder="name"
                            />
                            <input
                              value={gifEditCategory}
                              onChange={(e) => setGifEditCategory(e.target.value)}
                              placeholder="category"
                            />
                            <input
                              value={gifEditTags}
                              onChange={(e) => setGifEditTags(e.target.value)}
                            ...
                            <div className="server-menu__gif-popover-actions">
                              <button
                                ...
                                className="server-menu__gif-popover-btn server-menu__gif-popover-btn--cancel"
                              >
                                cancel
                              </button>
                              <button
                                onClick={handleSaveEdit}
                                disabled={!gifEditName.trim()}
                                className="server-menu__gif-popover-btn server-menu__gif-popover-btn--save"
                              >
                                save
                              </button>
                            </div>
        ...
        {gifTab === 'sticker' && (
          <>
            <p
              ...
              style={{ marginBottom: '6px', fontSize: '11px' }}
            >
              upload sticker pack (.zip)
            </p>
            <div className="server-menu__gif-upload" style={{ marginBottom: '16px' }}>
              <p className="server-menu__css-hint" style={{ marginBottom: '8px' }}>
                Upload a .zip containing .gif, .png, or .webp stickers. Optionally include a
                pack.json manifest.
              </p>
              <button
                onClick={() => stickerPackFileRef.current?.click()}
                disabled={gifUploading}
                className="server-menu__upload-btn"
              >
                {gifUploading ? 'uploading...' : 'choose .zip file'}
              ...
              <input
                ref={stickerPackFileRef}
                type="file"
                accept=".zip"
                style={{ display: 'none' }}
                onChange={handleStickerPackFile}
              />
            </div>
            ...
            {gifsLoading ? (
              ...
              (() => {
                const stickerPacks = [
                  ...new Set(
                    gifsList
                      .filter((g) => g.type === 'sticker' && g.pack_name)
                      .map((g) => g.pack_name!),
                  ),
                ]
                return stickerPacks.length === 0 ? (
                  ...
                  stickerPacks.map((pack) => {
                    const packCount = gifsList.filter(
                      (g) => g.type === 'sticker' && g.pack_name === pack,
                    ).length
                    return (
                      <div
                        key={pack}
                        className="server-menu__gif-item"
                        style={{
                          display: 'flex',
                          alignItems: 'center',
                      ...
                      >
                        <div>
                          ...
                          >
                            {packCount} sticker{packCount !== 1 ? 's' : ''}
                          </span>
                        </div>
      ...
      {(() => {
        const existingPacks = [
          ...new Set(
            gifsList.filter((g) => g.type === 'sticker' && g.pack_name).map((g) => g.pack_name!),
          ),
        ]
        ...
        return (
          ...
          >
            ...
            <div style={{ marginBottom: '12px' }}>
              <button
                className="server-menu__upload-btn"
                onClick={() => singleStickerFileRef.current?.click()}
              >
