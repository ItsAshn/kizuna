...
class AudioProcessorWorklet extends AudioWorkletProcessor {
  ...
  constructor() {
    ...
    this.gateReleaseCoeff = Math.exp(-1 / (0.12 * sampleRate)) // 120ms
    ...
    this.specReleaseCoeff = Math.exp(-1 / (0.1 * sampleRate))
    ...
    this.agcAttackCoeff = Math.exp(-1 / (0.05 * sampleRate)) // 50ms
    this.agcReleaseCoeff = Math.exp(-1 / (0.4 * sampleRate)) // 400ms
    this.agcRmsCoeff = Math.exp(-1 / (0.05 * sampleRate)) // 50ms EMA
  ...
  process(inputs, outputs, parameters) {
      ...
      for (let i = 0; i < inData.length; i++) {
        ...
        if (gateEnabled) {
          ...
          if (absSample > gateThreshold) {
            this.gateEnvelope =
              this.gateEnvelope * this.gateAttackCoeff + (1 - this.gateAttackCoeff)
        ...
        if (suppressionEnabled && suppressionStrength > 0) {
          ...
          for (let b = 0; b < 4; b++) {
            ...
            const coeff =
              absSig > this.bandEnvelope[b] ? this.specAttackCoeff : this.specReleaseCoeff
            ...
            if (this.bandEnvelope[b] < this.bandNoiseFloor[b]) {
              ...
              this.bandNoiseFloor[b] =
                this.bandNoiseFloor[b] * this.noiseLearnCoeff +
    ...
    if (agcEnabled && agcSampleCount > 0) {
      ...
      const desiredGain =
        this.agcAvgRms < 1e-10
          ? this.agcGain
          : Math.max(
              this.agcMaxAtten,
              Math.min(this.agcMaxGain, this.agcTargetRms / this.agcAvgRms),
            )

      const coeff = desiredGain > this.agcGain ? this.agcAttackCoeff : this.agcReleaseCoeff
