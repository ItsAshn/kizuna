...
pub fn list_monitors() -> Result<Vec<MonitorInfo>, String> {
    ...
    monitors
        ...
        .map(|(i, m)| {
            ...
            let width =
                m.width().map_err(|e| format!("Failed to get monitor width: {e}"))?;
            let height =
                m.height().map_err(|e| format!("Failed to get monitor height: {e}"))?;
            Ok(MonitorInfo {
                index: i,
                name,
                width,
...
pub fn start_capture(
    ...
    eprintln!(
        "[ScreenCapture] X11: capturing monitor '{}' at {} fps",
        monitor_name,
        ...
        loop {
            ...
            match capture_and_encode(&monitor) {
                ...
                Err(e) => {
                    ...
                    eprintln!(
                        "[ScreenCapture] X11 frame error ({} consecutive): {e}",
                        consecutive_errors
                    );
                    if consecutive_errors >= MAX_CONSECUTIVE_ERRORS {
                        eprintln!(
                            "[ScreenCapture] X11: too many consecutive errors ({}), stopping",
