...
| `JWT_SECRET` | _(generated)_      | Secret for signing auth tokens. Generate: `openssl rand -hex 64`           |
...
| `SERVER_URL`         | _(auto)_                         | Public HTTPS URL. Used for invite links. Should match your domain.                                                                                                                         |
| `SERVER_PASSWORD`    | _(none)_                         | Optional join password. If set, users must provide it to register.                                                                                                                         |
| `CORS_ORIGIN`        | _(none)_                         | Extra origins allowed to call this server from a browser, comma-separated. The official web app (`https://use-kizuna.com`), the desktop app, and localhost dev servers are always allowed. |

## WebRTC / Voice

| Variable              | Default           | Description                                                                                |
| --------------------- | ----------------- | ------------------------------------------------------------------------------------------ |
| `PUBLIC_ADDRESS`      | _(auto via STUN)_ | Public IP or hostname announced as WebRTC ICE candidate. Set this if auto-detection fails. |
